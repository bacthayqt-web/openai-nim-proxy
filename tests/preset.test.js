...
assert.strictEqual(preset.profile.internal_states.length, 0);
assert.strictEqual(preset.profile.continuity, 'visible_history_only');
...
assert(janitorSystem.content.includes('INTERNAL STATES ARE DISABLED ON EVERY FRONTEND'));
...
assert.strictEqual(genericHistory[2].content, 'Recent.', 'Newest generic state must also be removed');
